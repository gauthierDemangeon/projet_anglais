...
public class MatriceAdjacence {
	...
	public MatriceAdjacence(Graphe G){
		...
		Arc aTemp;
		for(int i=0;i<n;i++){
			for(int j=i+1;j<n;j++){ // MODIF : j=0 => j=i+1 (on ne crée pas les arcs dans les deux sens => graphe orienté)
				...
				Matrice[j][i]=Matrice[i][j]; // MODIF : Matrice symétrique
			}
		}
		this.n = n;
	}
    ...
    public static void main (String [] args) throws IOException 
	{
		...
		do
		{
			...
			gpsSommet(graphe.getG());
			//System.out.println("Nombre de sommet : " + M.Matrice.length + "( entre 0 et "+ (M.Matrice.length-1)+" )");
			do
			{
				System.out.println("Veuillez indiquer le sommet initial : ");
				Si = Integer.parseInt(scanInput.nextLine());
				System.out.println("Veuillez indiquer le sommet terminal : ");
	...
	/* MODIF : ajout fonction affiche chaque sommet avec ses coordonnées GPS */
